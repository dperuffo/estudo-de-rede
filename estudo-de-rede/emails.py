...
<div style="font-size:22px;font-weight:800;color:#fff;">FNI Gestão de Frotas</div>
...
<p style="margin:0;font-size:12px;color:#999;">FNI Gestão de Frotas &middot; <a href="{APP_URL}" style="color:#1040a0;">fxgestaodefrotasonline.com</a><br>
...
def enviar_boas_vindas(email: str, nome_empresa: str) -> bool:
    c = f"""<h2 style="color:#0d1b4b;margin-top:0;">Bem-vindo ao FNI Gestão de Frotas!</h2>
    ...
    return _enviar(email, "Bem-vindo ao FNI Gestão de Frotas! Seu trial comecou", _base(c, "Boas-vindas"))
...
def enviar_engajamento_d7(email: str, nome_empresa: str) -> bool:
    c = f"""<h2 style="color:#0d1b4b;margin-top:0;">7 dias de FNI Gestão de Frotas!</h2>
    ...
    return _enviar(email, "Uma semana de FNI Gestão de Frotas!", _base(c, "Engajamento D+7"))
