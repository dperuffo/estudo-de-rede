...
class _DetalheSheet extends StatefulWidget {
  ...
  @override
  State<_DetalheSheet> createState() => _State();
}
...
class _State extends State<_DetalheSheet> {
  ...
  Future<void> _load() async {
    ...
    if (id == null) {
      setState(() {
        _detalhes = Map<String, dynamic>.from(widget.abastecimento);
        _loading = false;
      });
      return;
    }
          ...
          child: Row(children: [
            Container(
              ...
              child: const Icon(Icons.local_gas_station,
                  color: AppTheme.glassTextoAtivo, size: 24),
            ),
            ...
            child: _loading
                ...
                : ListView(
                    ...
                    children: [
                        ...
                        Container(
                          ...
                          child: Row(
                              ...
                              children: [
                                _valorCard(
                                    'Valor Total',
                                    fmt.format(d['item_valor_total'] ?? 0),
                                    Colors.white),
                                _valorCard(
                                    'Litros',
                                    '${(d['item_quantidade'] ?? 0).toStringAsFixed(0)} L',
                                    Colors.white70),
                                _valorCard(
                                    'Preco/L',
                                    fmt.format(d['item_valor_unitario'] ?? 0),
                                    Colors.white70),
                              ]),
                        ),
                        ...
                        _linha('Identificador',
                            d['identificador']?.toString() ?? '-'),
                        _linha(
                            'Status', _formatarStatus(d['status_autorizacao']),
                            cor: _corStatus(d['status_autorizacao'])),
                        if (d['abastecimento_estornado'] == 1)
                          _linha('Estornado', 'Sim', cor: Colors.red),
                        if (d['motivo_recusa'] != null &&
                            d['motivo_recusa'].toString().isNotEmpty)
                        ...
                        if (d['hodometro'] != null &&
                            (d['hodometro'] as num) > 0)
                          _linha('Hodometro',
                              '${(d['hodometro'] as num).toStringAsFixed(0)} km'),
                        if (d['horimetro'] != null &&
                            (d['horimetro'] as num) > 0)
                          _linha('Horimetro',
                              '${(d['horimetro'] as num).toStringAsFixed(0)} h'),
                        const SizedBox(height: 16),
                        ...
                        _linha('Municipio',
                            '${d['pv_municipio'] ?? '-'}/${d['pv_uf'] ?? '-'}'),
                        if (d['pv_posto_interno'] != null &&
                            d['pv_posto_interno'] != false)
                          _linha('Posto Interno',
                              d['pv_posto_interno'].toString()),
                        if (d['pv_latitude'] != null &&
                            d['pv_longitude'] != null)
                          _linhaAcao('Ver no mapa', Icons.map, Colors.green,
                              _abrirMapa),
                        const SizedBox(height: 16),
                        ...
                        if (d['frota_razao_social'] != null) ...[
                          _secao('Empresa'),
                          _linha(
                              'Razao Social', d['frota_razao_social'] ?? '-'),
                          const SizedBox(height: 16),
                        ],
                        ...
                        _secao('Registro'),
                        _linha(
                            'Importado em', _formatarData(d['importado_em'])),
  ...
  Widget _valorCard(String label, String valor, Color cor) => Column(children: [
        Text(label,
            style: TextStyle(color: cor.withOpacity(0.7), fontSize: 11)),
  ...
  Color _corStatus(dynamic status) {
    ...
    if (s.contains('aprovad') || s.contains('autorizado') || s == '1')
      return Colors.green;
    if (s.contains('recusad') || s.contains('negado') || s == '0')
      return Colors.red;
