...
class _VeiculoSheet extends StatefulWidget {
  ...
  @override
  State<_VeiculoSheet> createState() => _State();
}
...
class _State extends State<_VeiculoSheet> {
  ...
  Future<void> _abrirFormulario(BuildContext ctx) async {
    ...
    final marcaCtrl =
        TextEditingController(text: cadastro['marca'] ?? fipe['marca'] ?? '');
    final modeloCtrl =
        TextEditingController(text: cadastro['modelo'] ?? fipe['modelo'] ?? '');
    final motorCtrl = TextEditingController(text: cadastro['motor'] ?? '');
    final anoModCtrl = TextEditingController(
        text: (cadastro['ano_modelo'] ?? fipe['ano_modelo'])?.toString() ?? '');
    final anoFabCtrl = TextEditingController(
        text: cadastro['ano_fabricacao']?.toString() ?? '');
    final corCtrl =
        TextEditingController(text: cadastro['cor'] ?? fipe['cor'] ?? '');
    final combCtrl = TextEditingController(
        text: cadastro['combustivel'] ?? fipe['combustivel_fipe'] ?? '');
    final tanqueCtrl = TextEditingController(
        text: (cadastro['tanque'] as num?)?.toStringAsFixed(0) ?? '');
    final autonomiaCtrl = TextEditingController(
        text: (cadastro['autonomia'] as num?)?.toStringAsFixed(1) ?? '');
    final hodCtrl = TextEditingController(
        text: (cadastro['hodometro_atual'] as num?)?.toStringAsFixed(0) ?? '');
              ...
              child: SingleChildScrollView(
                  ...
                  child: Column(children: [
                    ...
                    Row(children: [
                      ...
                      Expanded(
                          child: _campo(anoFabCtrl, 'Ano Fabricacao',
                    ...
                    Row(children: [
                      Expanded(
                          child: _campo(tanqueCtrl, 'Tanque (L)',
                              tipo: TextInputType.number)),
                      const SizedBox(width: 8),
                      Expanded(
                          child: _campo(autonomiaCtrl, 'Autonomia (km/L)',
                              tipo: TextInputType.number)),
                    ]),
                    _campo(hodCtrl, 'Hodometro Atual (km)',
                        tipo: TextInputType.number),
                            ...
                            try {
                              final body = {
                                ...
                                'ano_modelo': int.tryParse(anoModCtrl.text
                                    ...
                                    .substring(
                                        0,
                                        anoModCtrl.text.length > 4
                                            ? 4
                                            : anoModCtrl.text.length)),
                                'ano_fabricacao': int.tryParse(anoFabCtrl.text),
                                'cor': corCtrl.text.trim(),
                                'combustivel': combCtrl.text.trim(),
                                'tanque': double.tryParse(
                                    tanqueCtrl.text.replaceAll(',', '.')),
                                'autonomia': double.tryParse(
                                    autonomiaCtrl.text.replaceAll(',', '.')),
                                'hodometro_atual':
                                    double.tryParse(hodCtrl.text),
                              ...
                              if (idCadastro != null) {
                                await ApiService()
                                    .put('/veiculos/$idCadastro', data: body);
                              } else {
                                await ApiService()
                                    .post('/veiculos', data: body);
                              }
                              if (bCtx.mounted) {
                                Navigator.pop(bCtx);
                                if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final totalManut =
        (_dados?['total_manutencao_30d'] as num? ?? 0).toDouble();

    final marca = cadastro['marca'] ?? fipe['marca'] ?? '-';
    final modelo = cadastro['modelo'] ?? fipe['modelo'] ?? '-';
    final ano =
        (cadastro['ano_modelo'] ?? fipe['ano_modelo'])?.toString() ?? '';
    final cor = (cadastro['cor'] ?? fipe['cor'])?.toString() ?? '';
    final comb =
        (cadastro['combustivel'] ?? fipe['combustivel_fipe'])?.toString() ?? '';
                ...
                child: Column(
                  ...
                  Text(widget.placa,
                      style: const TextStyle(
                          ...
                          fontWeight: FontWeight.bold)),
                  if (marca != '-')
                    Text('$marca ${modelo != '-' ? modelo : ''}',
                        style: const TextStyle(
                            color: Colors.white70, fontSize: 13)),
                  if (ano.isNotEmpty || cor.isNotEmpty)
                    Text(
                        '${ano.isNotEmpty ? ano : ''} ${cor.isNotEmpty ? "· $cor" : ""}',
                        style: const TextStyle(
                ...
                : ListView(padding: const EdgeInsets.all(16), children: [
                    ...
                    if (resAbast.isNotEmpty) ...[
                      ...
                      Row(children: [
                        _kpi(
                            'Abastecimentos',
                            '${resAbast["n_abastecimentos"] ?? 0}',
                            Colors.blue),
                        const SizedBox(width: 8),
                        _kpi(
                            'Gasto Comb.',
                            fmt.format(resAbast['total_gasto'] ?? 0),
                            Colors.green),
                        const SizedBox(width: 8),
                        _kpi(
                            'Litros',
                            '${(resAbast["total_litros"] ?? 0).toStringAsFixed(0)} L',
                            Colors.cyan),
                      ]),
                      ...
                      Row(children: [
                        _kpi(
                            'Preco Medio/L',
                            fmt.format(resAbast['preco_medio'] ?? 0),
                            Colors.orange),
                        const SizedBox(width: 8),
                    ...
                    if (cadastro.isNotEmpty || fipe.isNotEmpty) ...[
                      ...
                      if (cadastro['chassi'] != null)
                        _linha('Chassi', cadastro['chassi']),
                      if (cadastro['renavam'] != null)
                        _linha('RENAVAM', cadastro['renavam']),
                      if (cadastro['hodometro_atual'] != null)
                        _linha('Hodometro',
                            '${NumberFormat("#,##0").format(cadastro["hodometro_atual"])} km'),
                      const SizedBox(height: 16),
                    ],
                    ...
                    if (fipe.isNotEmpty) ...[
                      _secao('Tabela FIPE'),
                      if (fipe['codigo_fipe'] != null)
                        _linha('Codigo FIPE', fipe['codigo_fipe']),
                      if (fipe['valor_fipe'] != null)
                        _linha('Valor FIPE', fmt.format(fipe['valor_fipe'])),
                      if (fipe['mes_referencia'] != null)
                        _linha('Referencia', fipe['mes_referencia']),
                      const SizedBox(height: 16),
                    ],
                    ...
                    if (ultAbast.isNotEmpty) ...[
                      ...
                      _linha(
                          'Data',
                          dataBr(ultAbast['data_abastecimento'],
                              comHora: true)),
                      _linha('Combustivel', ultAbast['item_nome'] ?? '-'),
                      _linha('Litros',
                          '${(ultAbast["item_quantidade"] ?? 0).toStringAsFixed(1)} L'),
                      _linha('Valor',
                          fmt.format(ultAbast['item_valor_total'] ?? 0)),
                      _linha('Preco/L',
                          fmt.format(ultAbast['item_valor_unitario'] ?? 0)),
                      if (ultAbast['hodometro'] != null &&
                          (ultAbast['hodometro'] as num) > 0)
                        _linha('Hodometro',
                            '${NumberFormat("#,##0").format(ultAbast["hodometro"])} km'),
                      _linha('Posto', '${ultAbast["pv_razao_social"] ?? "-"}'),
                      _linha('Local',
                          '${ultAbast["pv_municipio"] ?? "-"}/${ultAbast["pv_uf"] ?? "-"}'),
                      const SizedBox(height: 16),
                    ],
                    ...
                    if (ultManut.isNotEmpty) ...[
                      ...
                      if (ultManut['hodometro'] != null)
                        _linha('Hodometro',
                            '${NumberFormat("#,##0").format(ultManut["hodometro"])} km'),
                      if ((ultManut['itens_realizados'] as List?)?.isNotEmpty ==
                          true) ...[
                        const SizedBox(height: 8),
                        const Text('Servicos:',
                            style: TextStyle(fontSize: 12, color: Colors.grey)),
                        ...(ultManut['itens_realizados'] as List)
                            .map((item) => Padding(
                                  padding:
