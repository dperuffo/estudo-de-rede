...
class ChipsReputacaoMotorista extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Wrap(
      ...
      runSpacing: 4,
      children: [
        if (reputacao.seloVerificado)
          chip('✅ Motorista verificado', cor: Colors.green),
        // Fase Destaques-Automaticos — tags que se repetiram em 2+
        // avaliações de clientes diferentes (ver _reputacao_motorista).
        ...reputacao.tagsDestaque
            .map((t) => chip('🏷️ ${t.tag}', cor: Colors.blue)),
        chip(
        ...
        chip(reputacao.cnhValida ? '🪪 CNH válida' : '🪪 CNH vencida/ausente',
            cor: reputacao.cnhValida ? null : Colors.orange),
        chip(reputacao.telefoneVerificado
            ? '📱 Telefone verificado'
            : '📱 Não verificado'),
