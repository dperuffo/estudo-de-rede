...
class AcordosScreen extends StatefulWidget {
  ...
  @override
  State<AcordosScreen> createState() => _State();
}
...
class _State extends State<AcordosScreen> {
  ...
  Future<void> _novoOuEditar([Map? dados]) async {
    ...
    final precoCtrl = TextEditingController(
        text: dados?['preco_negociado']?.toString() ?? '');
    final inicioCtrl =
        TextEditingController(text: dados?['dt_vigencia_inicio'] ?? '');
    final fimCtrl =
        TextEditingController(text: dados?['dt_vigencia_fim'] ?? '');
        ...
        builder: (dialogCtx) => AlertDialog(
              ...
              content: SingleChildScrollView(
                ...
                TextField(
                    controller: cnpjCtrl,
                    decoration:
                        const InputDecoration(labelText: 'CNPJ do Posto *')),
                TextField(
                    controller: nomeCtrl,
                    decoration:
                        const InputDecoration(labelText: 'Nome do Posto')),
                TextField(
                    controller: combCtrl,
                    decoration:
                        const InputDecoration(labelText: 'Combustivel *')),
                TextField(
                    controller: precoCtrl,
                    decoration: const InputDecoration(
                        labelText: 'Preco Negociado (R\$)'),
                    keyboardType: TextInputType.number),
                TextField(
                    controller: inicioCtrl,
                    decoration: const InputDecoration(
                        labelText: 'Inicio vigencia (YYYY-MM-DD)')),
                TextField(
                    controller: fimCtrl,
                    decoration: const InputDecoration(
                        labelText: 'Fim vigencia (YYYY-MM-DD)')),
              ])),
              actions: [
                TextButton(
                ...
                ElevatedButton(
                  onPressed: () async {
                    try {
                      final body = {
                        'cnpj_posto':
                            cnpjCtrl.text.replaceAll(RegExp(r'\D'), ''),
                        'nome_posto': nomeCtrl.text.trim(),
                        'combustivel': combCtrl.text.trim(),
                        'preco_negociado': double.tryParse(
                      ...
                      if (dados == null) {
                        ...
                        await ApiService()
                            .put('/acordos/${dados["id"]}', data: body);
                      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
          ...
          title: const Text('Acordos de Preco')),
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            trailing: Column(
                                ...
                                children: [
                                  Text(
                                      a['preco_negociado'] != null
                                          ? fmt.format(a['preco_negociado'])
                                          : '-',
                                      style: const TextStyle(
                                          fontWeight: FontWeight.bold,
                                          color: Colors.green,
