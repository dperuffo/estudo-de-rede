...
class ClienteCrmDetalheScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ClienteCrmDetalheScreen> createState() => _ClienteCrmDetalheScreenState();
}

class _ClienteCrmDetalheScreenState extends ConsumerState<ClienteCrmDetalheScreen> {
  bool _processando = false;
  String? _erro;

  String _fmtMoeda(double v) => 'R\$ ${v.toStringAsFixed(2).replaceAll('.', ',')}';
  ...
  Future<void> _registrarInteracao(String empresaId, String tipo, String descricao, String? proximaAcaoData) async {
    setState(() {
      _erro = null;
      _processando = true;
    });
  ...
  Widget _conteudo(ClienteCrm c) {
    ...
    return ListView(
      ...
      children: [
        ...
        Text(c.razaoSocial, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
        const SizedBox(height: 2),
        Text(_fmtCnpjCpf(c.cnpjCpf), style: const TextStyle(fontSize: 12, color: Colors.grey)),
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Text('Funil de propostas', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                const SizedBox(height: 10),
                cotacoesAsync.when(
                  data: (lista) {
                    final aberto = lista.where((c) => c.status == 'simulada').fold<double>(0, (s, c) => s + c.valorTotal);
                    final ganho = lista.where((c) => c.status == 'convertida').fold<double>(0, (s, c) => s + c.valorTotal);
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Row(
                          children: [
                            ...
                            _campoResumo('Ganho', _fmtMoeda(ganho), destaque: true),
                        ...
                        if (lista.isEmpty)
                          const Padding(
                            ...
                            child: Text('Nenhuma proposta ainda.', style: TextStyle(fontSize: 12, color: Colors.grey)),
                          )
                        else ...[
                          const SizedBox(height: 10),
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Registrar interação', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                const SizedBox(height: 10),
                _FormInteracao(
                  processando: _processando,
                  onRegistrar: (tipo, descricao, proximaAcao) => _registrarInteracao(c.empresaId, tipo, descricao, proximaAcao),
                ),
                const SizedBox(height: 14),
                const Divider(),
                const SizedBox(height: 6),
                const Text('Histórico de relacionamento', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                const SizedBox(height: 10),
                interacoesAsync.when(
                  data: (lista) {
                    if (lista.isEmpty) {
                      return const Text('Nenhuma interação registrada ainda.', style: TextStyle(fontSize: 12, color: Colors.grey));
                    }
                    return Column(children: lista.map(_linhaInteracao).toList());
                  },
                  loading: () => const Center(child: Padding(padding: EdgeInsets.all(8), child: CircularProgressIndicator())),
                  error: (e, _) => Text('Erro: $e', style: const TextStyle(fontSize: 12, color: Colors.red)),
                ),
    ...
    return Expanded(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 2),
          Text(valor, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: destaque ? const Color(0xFF166534) : Colors.black87)),
        ],
      ),
    );
  }
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('${cot.origemLabel} → ${cot.destinoLabel}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                Text('${_fmtMoeda(cot.valorTotal)} · ${_fmtData(cot.criadoEm)}', style: const TextStyle(fontSize: 11, color: Colors.grey)),
              ],
            ),
          ),
          Container(
            ...
            child: Text(statusPropostaLabel[cot.status] ?? cot.status, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: cor)),
          ),
        ],
      ),
    );
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(tipoInteracaoLabel[i.tipo] ?? i.tipo, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
              Text(_fmtDataHora(i.criadoEm), style: const TextStyle(fontSize: 10, color: Colors.grey)),
            ],
          ),
              ...
              Expanded(
                child: Text(
                  [
                    ...
                    if (i.proximaAcaoData != null) 'Próxima ação: ${_fmtData(i.proximaAcaoData!)}',
                  ].join(' · '),
...
class _FormInteracao extends StatefulWidget {
  ...
  final Future<void> Function(String tipo, String descricao, String? proximaAcaoData) onRegistrar;
...
class _FormInteracaoState extends State<_FormInteracao> {
  ...
  Future<void> _enviar() async {
    ...
    await widget.onRegistrar(_tipo!, _descricaoCtrl.text.trim(), proximaAcaoIso);
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          items: tipoInteracaoLabel.entries.map((e) => DropdownMenuItem(value: e.key, child: Text(e.value))).toList(),
        ...
        TextField(
          ...
          maxLines: 2,
          decoration: const InputDecoration(labelText: 'O que foi conversado/combinado', border: OutlineInputBorder(), isDense: true),
        ),
        const SizedBox(height: 10),
        InkWell(
          onTap: _selecionarData,
          child: InputDecorator(
            decoration: const InputDecoration(labelText: 'Próxima ação (opcional)', border: OutlineInputBorder(), isDense: true, suffixIcon: Icon(Icons.calendar_today, size: 16)),
            child: Text(
              _proximaAcao == null
                  ? '—'
        ...
        SizedBox(
          ...
          child: FilledButton(
            ...
            child: Text(widget.processando ? 'Registrando...' : 'Registrar Interação'),
          ),
        ),
      ],
    );
