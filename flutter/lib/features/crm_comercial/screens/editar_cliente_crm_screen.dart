...
class EditarClienteCrmScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<EditarClienteCrmScreen> createState() =>
      _EditarClienteCrmScreenState();
}

class _EditarClienteCrmScreenState
    extends ConsumerState<EditarClienteCrmScreen> {
  ...
  Future<void> _salvar() async {
    ...
    try {
      await CrmService().editarCliente(
        ...
        enderecoMunicipio: _municipioCtrl.text.trim().isEmpty
            ? null
            : _municipioCtrl.text.trim(),
        enderecoUf: _ufCtrl.text.trim().isEmpty
            ? null
            : _ufCtrl.text.trim().toUpperCase(),
        enderecoCep: _cepCtrl.text.trim().isEmpty
            ? null
            : _cepCtrl.text.replaceAll(RegExp(r'\D'), ''),
      );
      if (!mounted) return;
