...
class _EstoquePecasScreenState extends ConsumerState<EstoquePecasScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Estoque de Peças')),
        ...
        child: pecasAsync.when(
          data: (lista) {
            ...
            final filtradas = termo.isEmpty
                ? lista
                : lista.where((p) => p.nome.toLowerCase().contains(termo) || (p.codigo?.toLowerCase().contains(termo) ?? false)).toList();

            final ativas = lista.where((p) => p.ativa).toList();
            final abaixoDoMinimo = ativas.where((p) => p.abaixoDoMinimo).length;
            final valorEmEstoque = ativas.fold<double>(0, (s, p) => s + p.quantidadeAtual * (p.custoUnitarioMedio ?? 0));
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    _kpi('Abaixo do mínimo', '$abaixoDoMinimo', destaque: abaixoDoMinimo > 0),
                    const SizedBox(width: 8),
                ...
                if (filtradas.isEmpty)
                  const Padding(
                    ...
                    child: Center(child: Text('Nenhuma peça cadastrada ainda.', style: TextStyle(color: Colors.grey))),
                  )
                else
                  ...filtradas.map(_card),
              ],
      ...
      child: ListTile(
        ...
        title: Row(
          children: [
            ...
            if (p.codigo != null) Text(p.codigo!, style: const TextStyle(fontSize: 11, color: Colors.grey)),
          ],
        ),
        subtitle: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                ...
                style: TextStyle(fontSize: 12, color: p.abaixoDoMinimo ? const Color(0xFFB91C1C) : Colors.black87, fontWeight: p.abaixoDoMinimo ? FontWeight.w700 : FontWeight.normal),
              ),
              const SizedBox(height: 2),
              Text('Custo médio: ${_fmtMoeda(p.custoUnitarioMedio)}', style: const TextStyle(fontSize: 11, color: Colors.grey)),
              const SizedBox(height: 6),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                decoration: BoxDecoration(
                  color: p.abaixoDoMinimo ? const Color(0xFFFEE2E2) : const Color(0xFFDCFCE7),
                  borderRadius: BorderRadius.circular(12),
                ),
                child: Text(
                  p.abaixoDoMinimo ? 'Repor' : 'OK',
                  style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: p.abaixoDoMinimo ? const Color(0xFF991B1B) : const Color(0xFF166534)),
                ),
              ),
            ],
          ),
