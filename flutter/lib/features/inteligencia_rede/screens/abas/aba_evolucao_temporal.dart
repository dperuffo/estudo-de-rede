...
class _AbaEvolucaoTemporalState extends State<AbaEvolucaoTemporal> {
  ...
  Widget build(BuildContext context) {
    ...
    final combustiveis = registros.map((r) => r.combustivel).toSet().toList()
      ..sort();
    final ufs = registros.map((r) => r.uf).whereType<String>().toSet().toList()
      ..sort();

    final filtrado = registros.where((r) {
      if (_combustivelSel != 'Todos' && r.combustivel != _combustivelSel)
        return false;
      ...
      child: Column(
        ...
        children: [
          Wrap(spacing: 12, runSpacing: 8, children: [
            _dropdown('Combustível', ['Todos', ...combustiveis],
                _combustivelSel, (v) => setState(() => _combustivelSel = v)),
            _dropdown('UF', ['Todos', ...ufs], _ufSel,
                (v) => setState(() => _ufSel = v)),
            _dropdownBool('Granularidade', {'Semanal': true, 'Mensal': false},
                _semanal, (v) => setState(() => _semanal = v)),
          ]),
  ...
  Widget _conteudo(List<RegistroPrecoHistorico> filtrado) {
    ...
    var tendenciaPorUf = mapa.entries
        .map((e) => (
              uf: e.key,
              pontos: e.value.entries
                  .map((p) =>
                      (periodo: p.key, precoMedio: p.value.soma / p.value.qtd))
                  .toList()
    ...
    tendenciaPorUf = tendenciaPorUf.take(10).toList()
      ..sort((a, b) => a.uf.compareTo(b.uf));
    ...
    for (final p in widget.dados.precoRealPeriodo) {
      ...
      precoRealMapa[periodo] =
          (soma: atual.soma + p.precoMedio * p.qtd, qtd: atual.qtd + p.qtd);
    }
    final precoRealSerie = precoRealMapa.entries
        .map((e) => (
              periodo: e.key,
              precoReal: e.value.qtd > 0 ? e.value.soma / e.value.qtd : 0.0
            ))
        .toList()
      ..sort((a, b) => a.periodo.compareTo(b.periodo));
    ...
    if (tendenciaPorUf.isNotEmpty) {
      final deltas = tendenciaPorUf.map((s) {
        ...
        final pct =
            primeiro != 0 ? ((ultimo - primeiro) / primeiro) * 100 : 0.0;
        return (uf: s.uf, pct: pct);
      }).toList()
        ..sort((a, b) => b.pct.compareTo(a.pct));
      insights = (
        alta: (deltas.first.uf, deltas.first.pct),
        queda: (deltas.last.uf, deltas.last.pct)
      );
    }

    // ---- Volatilidade por UF ----
    final linhasVol = mapa.entries
        .map((e) {
          final mediasPeriodo =
              e.value.values.map((v) => v.soma / v.qtd).toList();
          return (
            uf: e.key,
            media: media(mediasPeriodo),
            std: desvioPadraoAmostral(mediasPeriodo),
            n: mediasPeriodo.length
          );
        })
        .where((l) => l.n >= 2)
        .map((l) => (
              ...
              std: l.std,
              n: l.n,
              cv: l.media != 0 ? (l.std / l.media) * 100 : 0.0
            ))
        .toList();
    ...
    final ranking = postosMapa.entries
        .map((e) {
          ...
          return (
            cnpj: e.key,
            razaoSocial: e.value.razaoSocial,
            municipio: e.value.municipio,
            uf: e.value.uf,
            n: e.value.precos.length,
            media: m,
            std: std,
            cvPct: m != 0 ? (std / m) * 100 : 0.0
          );
        })
        .where((l) => l.n >= 3)
        .toList()
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('🗺️ Tendência de preço por UF',
            style: TextStyle(
                fontSize: 11,
        ...
        if (tendenciaPorUf.isEmpty)
              ...
              LineChartData(
                ...
                lineBarsData: [
                  ...tendenciaPorUf.asMap().entries.map((e) {
                    final idxPorPeriodo = {
                      for (final p in e.value.pontos) p.periodo: p.precoMedio
                    };
                    return LineChartBarData(
                      spots: periodosOrdenados
                          .asMap()
                          .entries
                          .where((pe) => idxPorPeriodo.containsKey(pe.value))
                          .map((pe) => FlSpot(
                              pe.key.toDouble(), idxPorPeriodo[pe.value]!))
                          .toList(),
                      isCurved: false,
                  ...
                  if (precoRealSerie.isNotEmpty)
                    LineChartBarData(
                      spots: periodosOrdenados
                          ...
                          .entries
                          .where((pe) =>
                              precoRealSerie.any((p) => p.periodo == pe.value))
                          .map((pe) => FlSpot(
                              pe.key.toDouble(),
                              precoRealSerie
                                  .firstWhere((p) => p.periodo == pe.value)
                                  .precoReal))
                          .toList(),
                      isCurved: false,
                      color: _corPrecoReal,
                ...
                titlesData: FlTitlesData(
                  ...
                  bottomTitles: AxisTitles(
                      sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (v, _) {
                            final i = v.toInt();
                            if (i < 0 || i >= periodosOrdenados.length)
                              return const SizedBox.shrink();
                            return Text(
                                _periodoLabel(periodosOrdenados[i], _semanal),
          ...
          Wrap(spacing: 10, runSpacing: 4, children: [
            ...tendenciaPorUf.asMap().entries.map((e) =>
                _legendaLinha(_coresUf[e.key % _coresUf.length], e.value.uf)),
            if (precoRealSerie.isNotEmpty)
              _legendaLinha(_corPrecoReal, '💰 Preço real pago'),
          ]),
          if (limitado)
          ...
          if (insights != null) ...[
            ...
            BlocoInsight(
                texto:
                    '${insights.alta.$2 > 0 ? "📈" : "📉"} ${insights.alta.$1} — variação de ${insights.alta.$2 >= 0 ? "+" : ""}${insights.alta.$2.toStringAsFixed(1)}% (maior alta)'),
            BlocoInsight(
                texto:
                    '${insights.queda.$2 < 0 ? "📉" : "📈"} ${insights.queda.$1} — variação de ${insights.queda.$2 >= 0 ? "+" : ""}${insights.queda.$2.toStringAsFixed(1)}% (maior queda)'),
          ],
        ],
        const SizedBox(height: 20),
        Text('🌊 Volatilidade de preços por UF',
            style: TextStyle(
                fontSize: 11,
                fontWeight: FontWeight.w700,
                color: Colors.grey.shade600)),
          ...
          BarraHorizontal(
              dados: porStd
                  .map((v) => BarraHorizontalItem(
                      ...
                      texto: formatarMoeda(v.std, casas: 4)))
                  .toList(),
              eixoX: 'Desvio padrão (R\$/L)'),
          const SizedBox(height: 12),
          Text('Coeficiente de variação por UF',
              style:
                  const TextStyle(fontSize: 11, fontWeight: FontWeight.w600)),
          BarraHorizontal(
              dados: porCv
        ...
        if (ranking.isEmpty)
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, i) {
                final p = ranking[i];
                final medalha = ['🥇', '🥈', '🥉'].length > i
                    ? ['🥇', '🥈', '🥉'][i]
                    : '#${i + 1}';
                final (bg, borda) = p.cvPct < 1
                    ? (const Color(0xFFE8F5E9), const Color(0xFF43A047))
                    : (p.cvPct < 3
                        ? (const Color(0xFFFFF8E1), const Color(0xFFF57C00))
                        : (const Color(0xFFFCE4EC), const Color(0xFFE53935)));
                return Container(
                  width: 140,
                  ...
                  child: Column(
                      ...
                      children: [
                        ...
                        Text(truncarTexto(p.razaoSocial, 20),
          ...
          const SizedBox(height: 16),
          Text('📊 Distribuição de estabilidade (top 20)',
              style:
                  const TextStyle(fontSize: 11, fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          BarraHorizontal(
            dados: ranking
                ...
                .map((p) => BarraHorizontalItem(
                    label: '${truncarTexto(p.razaoSocial, 22)} (${p.uf})',
                    valor: p.cvPct,
                    cor: p.cvPct < 1
                        ? const Color(0xFF43A047)
                        : (p.cvPct < 3
                            ? const Color(0xFFF57C00)
                            : const Color(0xFFE53935)),
                    texto: '${p.cvPct.toStringAsFixed(2)}%'))
                .toList(),
            eixoX: 'Coeficiente de variação (%)',
          ),
          const SizedBox(height: 16),
          Text('📋 Tabela completa de estabilidade',
              style:
                  const TextStyle(fontSize: 11, fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          TabelaSimples(
            ...
            linhas: ranking
                .map((p) => [
                      truncarTexto(p.razaoSocial, 22),
                      p.uf,
                      '${p.n}',
                      formatarMoeda(p.media, casas: 4),
                      '${p.cvPct.toStringAsFixed(2)}%'
                    ])
                .toList(),
          ),
        ],
  ...
  Widget _legendaLinha(Color cor, String label) =>
  ...
  Widget _dropdownBool(String label, Map<String, bool> opcoes, bool valor,
