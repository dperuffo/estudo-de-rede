...
class _AbaCruzamentosState extends State<AbaCruzamentos> {
  ...
  Widget build(BuildContext context) {
    final combustiveis = widget.dados.precosPorUf
        .map((p) => p.combustivel)
        .toSet()
        .toList()
      ..sort();
      ...
      children: [
        ...
        Expanded(
          child: IndexedStack(
            ...
            children: [
              _RegioesCarasBaratas(
                  dados: widget.dados, combustiveis: combustiveis),
              _ClustersOportunidade(
                  dados: widget.dados, combustiveis: combustiveis),
              _GfVsConcorrencia(
                  dados: widget.dados, combustiveis: combustiveis),
...
class _RegioesCarasBaratasState extends State<_RegioesCarasBaratas> {
  ...
  Widget build(BuildContext context) {
    ...
    final atual = (_sel != null && combustiveis.contains(_sel))
        ? _sel!
        : (combustiveis.isNotEmpty ? combustiveis[0] : '');

    final filtrado = widget.dados.precosPorUf
        .where((p) => p.combustivel == atual)
        .toList()
            ...
            Builder(builder: (context) {
              ...
              final linhas = filtrado
                  .map((p) => (
                        ...
                        categoria: p.precoMedio >= p75
                            ? '🔴 Caro'
                            : (p.precoMedio <= p25 ? '🟢 Barato' : '🟡 Médio'),
                        cor: p.precoMedio >= p75
              ...
              final spreadPct = maisBarata.precoMedio != 0
                  ? (spread / maisBarata.precoMedio) * 100
                  : 0;
                ...
                children: [
                  Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Expanded(
                        child: _cardDestaque(
                            '🔴 UF mais cara',
                            maisCara.uf ?? '—',
                            formatarMoeda(maisCara.precoMedio),
                            '${maisCara.qtdPostos} postos',
                    ...
                    Expanded(
                        child: _cardDestaque(
                            '🟢 UF mais barata',
                            maisBarata.uf ?? '—',
                            formatarMoeda(maisBarata.precoMedio),
                            '${maisBarata.qtdPostos} postos',
                            const Color(0xFFF3FFF3),
                            const Color(0xFF43A047))),
                  ]),
                  const SizedBox(height: 6),
                  _cardDestaque(
                      '↕️ Spread entre extremos',
                      formatarMoeda(spread),
                      '${spreadPct.toStringAsFixed(1)}% de diferença',
                      null,
                  ...
                  SizedBox(
                    ...
                    child: BarChart(
                      BarChartData(
                        barTouchData:
                            barTouchPadrao(formatarY: (v) => formatarMoeda(v)),
                  ...
                  TabelaSimples(
                    ...
                    linhas: linhas
                        .map((l) => [
                              l.uf ?? '—',
                              l.categoria,
                              formatarMoeda(l.precoMedio, casas: 4),
                              '${l.qtdPostos}'
                            ])
                        .toList(),
                  ),
...
class _ClustersOportunidade extends StatefulWidget {
  final InteligenciaRedeCompleta dados;
  final List<String> combustiveis;
  const _ClustersOportunidade(
      {required this.dados, required this.combustiveis});
  @override
...
class _ClustersOportunidadeState extends State<_ClustersOportunidade> {
  ...
  Widget build(BuildContext context) {
    ...
    for (final r in widget.dados.historicoDetalhado) {
      if (r.combustivel != atual || r.uf == null || r.municipio == null)
        continue;
      ...
      mapa[chave] = (
        uf: at.uf,
        municipio: at.municipio,
        postos: at.postos,
        soma: at.soma + r.preco,
        qtd: at.qtd + 1
      );
    }
    final linhasBase = mapa.values
        .map((v) => (
              uf: v.uf,
              municipio: v.municipio,
              precoMedio: v.soma / v.qtd,
              postos: v.postos.length
            ))
        .toList();
      ...
      child: Column(
        ...
        children: [
          ...
          if (linhasBase.isEmpty)
            ...
            Builder(builder: (context) {
              final mediaNac =
                  media(linhasBase.map((l) => l.precoMedio).toList());
              final linhas = linhasBase.map((l) {
                final deltaVsMedia = mediaNac != 0
                    ? ((l.precoMedio - mediaNac) / mediaNac) * 100
                    : 0.0;
                return (
                  uf: l.uf,
                  ...
                  postos: l.postos,
                  deltaVsMedia: deltaVsMedia,
                  cluster: _classificarCluster(deltaVsMedia)
                );
              }).toList();

              final contagemCluster = <String, int>{};
              for (final l in linhas) {
                contagemCluster[l.cluster] =
                    (contagemCluster[l.cluster] ?? 0) + l.postos;
              }
              final ordenadoPorDelta = [...linhas]
                ..sort((a, b) => a.deltaVsMedia.compareTo(b.deltaVsMedia));
              final top15 = ordenadoPorDelta.take(15).toList();

              return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Distribuição por cluster',
                      style: TextStyle(
                          fontSize: 11,
                  ...
                  const SizedBox(height: 12),
                  Text('Top 15 municípios mais baratos',
                      style: TextStyle(
                          fontSize: 11,
                          fontWeight: FontWeight.w700,
                          color: Colors.grey.shade600)),
                  ...
                  BarraHorizontal(
                    dados: top15
                        .map((m) => BarraHorizontalItem(
                              label:
                                  '${truncarTexto(m.municipio, 18)} (${m.uf})',
                              valor: m.deltaVsMedia.abs(),
                              cor: m.deltaVsMedia < 0
                                  ? const Color(0xFF43A047)
                                  : const Color(0xFFF57C00),
                              texto:
                                  '${m.deltaVsMedia >= 0 ? "+" : ""}${m.deltaVsMedia.toStringAsFixed(1)}%',
                            ))
                        .toList(),
                    eixoX: 'Δ% vs média nacional',
                  ),
                  const SizedBox(height: 16),
                  Text('📋 Tabela completa de municípios',
                      style: TextStyle(
                          fontSize: 11,
                          fontWeight: FontWeight.w700,
                          color: Colors.grey.shade600)),
                  ...
                  TabelaSimples(
                    ...
                    linhas: ordenadoPorDelta
                        .map((m) => [
                              m.municipio,
                              m.uf,
                              formatarMoeda(m.precoMedio, casas: 4),
                              '${m.deltaVsMedia >= 0 ? "+" : ""}${m.deltaVsMedia.toStringAsFixed(2)}%',
                              '${m.postos}'
                            ])
                        .toList(),
                  ),
...
class _GfVsConcorrenciaState extends State<_GfVsConcorrencia> {
  ...
  Widget build(BuildContext context) {
    ...
    final mapa =
        <String, ({double somaGf, double somaAnp, int n, String nivel})>{};
    for (final dv in widget.dados.desvioAnp) {
      if (dv.combustivel != atual || dv.uf == null || dv.precoAnp == null)
        continue;
      final at = mapa[dv.uf!] ??
          (somaGf: 0.0, somaAnp: 0.0, n: 0, nivel: dv.nivelAnp ?? '—');
      mapa[dv.uf!] = (
        somaGf: at.somaGf + dv.precoGf,
        somaAnp: at.somaAnp + dv.precoAnp!,
        n: at.n + 1,
        nivel: at.nivel
      );
    }
    ...
    final deltaMedio = comp.isNotEmpty
        ? comp.fold<double>(0, (s, c) => s + c.deltaPct) / comp.length
        : 0.0;
    final alertas = comp.where((c) => c.deltaPct > 5).toList()
      ..sort((a, b) => b.deltaPct.compareTo(a.deltaPct));
    final oportunidades = comp.where((c) => c.deltaPct < -2).toList()
          ...
          if (comp.isEmpty)
            ...
            GridView.count(
              ...
              children: [
                CartaoIndicador(
                    label: '🔴 UFs caras (>+5%)',
                    valor: formatarInt(nCaros),
                    mini: true),
                CartaoIndicador(
                    label: '🟢 UFs baratas (<-2%)',
                    valor: formatarInt(nBaratos),
                    mini: true),
                CartaoIndicador(
                    label: '🟡 Faixa competitiva',
                    valor: formatarInt(nOk),
                    mini: true),
                CartaoIndicador(
                    label: '📊 Delta médio',
                    valor:
                        '${deltaMedio >= 0 ? "+" : ""}${deltaMedio.toStringAsFixed(1)}%',
                    mini: true),
              ],
            ),
            const SizedBox(height: 12),
            SizedBox(
              ...
              child: BarChart(
                BarChartData(
                  ...
                  extraLinesData: ExtraLinesData(horizontalLines: [
                    HorizontalLine(
                        y: 0, color: const Color(0xFF1040A0), strokeWidth: 1.5)
                  ]),
                  titlesData: FlTitlesData(
                    leftTitles: AxisTitles(
                        sideTitles: SideTitles(
            ...
            TabelaSimples(
              ...
              linhas: comp
                  .map((c) => [
                        c.uf,
                        formatarMoeda(c.gfMed, casas: 4),
                        formatarMoeda(c.anpMed, casas: 4),
                        '${c.deltaPct >= 0 ? "+" : ""}${c.deltaPct.toStringAsFixed(2)}%',
                        '${c.postos}'
                      ])
                  .toList(),
            ),
...
class _FrotaReal extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ufsCobertas =
        postosVisitados.map((p) => p.uf).where((v) => v != null).toSet().length;
    final precoMedioPago = totalVisitas > 0
        ? postosVisitados.fold<double>(
                0, (s, p) => s + p.precoMedio * p.visitas) /
            totalVisitas
        : 0.0;
    ...
    final pontosMapa =
        postosVisitados.where((p) => p.lat != null && p.lon != null).map((p) {
      final norm = (p.precoMedio - min) / faixa;
      final cor = norm < 0.33
          ? const Color(0xFF43A047)
          : (norm < 0.66 ? const Color(0xFFF57C00) : const Color(0xFFE53935));
      return PontoCirculo(
        lat: p.lat!,
        lon: p.lon!,
        cor: cor,
    ...
    for (final p in postosVisitados) {
      ...
      porUfMapa[p.uf!] = (
        soma: at.soma + p.precoMedio * p.visitas,
        visitas: at.visitas + p.visitas
      );
    }
    final porUf = porUfMapa.entries.map((e) {
      final precoReal =
          e.value.visitas > 0 ? e.value.soma / e.value.visitas : 0.0;
      final anpRef = dados.dieselAnpPorUf[e.key];
      final deltaPct = anpRef != null && anpRef != 0
          ? ((precoReal - anpRef) / anpRef) * 100
          : null;
      return (
        uf: e.key,
        precoReal: precoReal,
        visitas: e.value.visitas,
        anpRef: anpRef,
        deltaPct: deltaPct
      );
    }).toList()
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          GridView.count(
            ...
            children: [
              CartaoIndicador(
                  ...
                  valor: formatarInt(totalVisitas),
                  mini: true),
              CartaoIndicador(
                  label: '📍 Postos distintos',
                  valor: formatarInt(postosVisitados.length),
                  mini: true),
              CartaoIndicador(
                  label: '🗺️ UFs cobertas',
                  valor: formatarInt(ufsCobertas),
                  mini: true),
              CartaoIndicador(
                  label: '💰 Preço médio pago',
                  valor: formatarMoeda(precoMedioPago),
                  mini: true),
            ],
          ),
          const SizedBox(height: 12),
          Text('🌎 Mapa de calor — postos visitados pela frota',
              style: TextStyle(
                  fontSize: 11,
                  fontWeight: FontWeight.w700,
                  color: Colors.grey.shade600)),
          ...
          const SizedBox(height: 16),
          Text('🏆 Ranking de postos mais utilizados',
              style: TextStyle(
                  fontSize: 11,
                  fontWeight: FontWeight.w700,
                  color: Colors.grey.shade600)),
          ...
          BarraHorizontal(
            dados: top15Ranking
                .map((p) => BarraHorizontalItem(
                    label: truncarTexto(p.razaoSocial ?? p.cnpj, 28),
                    valor: p.visitas.toDouble(),
                    cor: const Color(0xFF283593),
                    texto: formatarInt(p.visitas)))
                .toList(),
            eixoX: 'Abastecimentos',
          ),
          const SizedBox(height: 16),
          Text('📊 Preço pago por UF (ref.: Diesel S10 ANP)',
              style: TextStyle(
                  fontSize: 11,
                  fontWeight: FontWeight.w700,
                  color: Colors.grey.shade600)),
          ...
          ...porUf.map((u) => Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _barraDupla(u.precoReal, u.anpRef ?? 0,
          ...
          TabelaSimples(
            ...
            linhas: porUf
                .map((u) => [
                      u.uf,
                      formatarMoeda(u.precoReal),
                      '${u.visitas}',
                      u.anpRef != null ? formatarMoeda(u.anpRef!) : '—',
                      u.deltaPct != null
                          ? '${u.deltaPct! >= 0 ? "+" : ""}${u.deltaPct!.toStringAsFixed(1)}%'
                          : '—'
                    ])
                .toList(),
          ),
...
Widget _cardDestaque(String titulo, String valor, String linha1, String? linha2,
    Color bg, Color borda) {
  return Container(
    decoration: BoxDecoration(
        color: bg,
...
Widget _seletorCombustivel(
    List<String> opcoes, String atual, ValueChanged<String> onChanged) {
  return Padding(
