...
(double score, String grade) _calcularScore(
    double diffPctSigned, int nServicos) {
  ...
  static List<_ScoreLinha> _calcularScores(InteligenciaRedeCompleta d) {
    ...
    for (final entry in porCnpj.entries) {
      ...
      resultado.add((
        ...
        uf: preferida.uf!,
        macro: regioesBrasil.entries
            .firstWhere((e) => e.value.contains(preferida.uf),
                orElse: () => const MapEntry('Outros', []))
            .key,
        score: score,
        grade: grade,
      ));
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        Expanded(
          child:
              IndexedStack(index: _subAba, sizing: StackFit.expand, children: [
...
class _MapaPrecosState extends State<_MapaPrecos> {
  ...
  Widget build(BuildContext context) {
    final combustiveis =
        widget.precosMapa.map((p) => p.combustivel).toSet().toList()..sort();
    final atual = (_sel != null && combustiveis.contains(_sel))
        ? _sel!
        : (combustiveis.isNotEmpty ? combustiveis[0] : '');
    final filtrado = widget.precosMapa
        .where((p) =>
            p.combustivel == atual &&
            p.lat != null &&
            p.lon != null &&
    ...
    final pontos = filtrado.map((p) {
      ...
      final cor = norm < 0.33
          ...
          : (norm < 0.66 ? const Color(0xFFF39C12) : const Color(0xFFE74C3C));
      return PontoCirculo(
          lat: p.lat!,
          lon: p.lon!,
          cor: cor,
        ...
        GridView.count(
          ...
          children: [
            CartaoIndicador(
                ...
                valor: formatarInt(filtrado.length),
                mini: true),
            CartaoIndicador(
                label: '💰 Mín', valor: formatarMoeda(min), mini: true),
            CartaoIndicador(
                label: '💰 Máx', valor: formatarMoeda(max), mini: true),
            CartaoIndicador(
                label: '📊 Mediana', valor: formatarMoeda(med), mini: true),
          ],
        ),
...
class _PostosInconsistentesState extends State<_PostosInconsistentes> {
  ...
  @override
  Widget build(BuildContext context) {
    final filtrado = widget.desvios
        .where((d) => d.diffPct.abs() > _tolerancia)
        .toList()
      ..sort((a, b) => b.diffPct.abs().compareTo(a.diffPct.abs()));
    final top20 = filtrado.take(20).toList();

    return SingleChildScrollView(
      padding: const EdgeInsets.all(12),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Tolerância de desvio vs ANP: ${_tolerancia.toInt()}%',
        ...
        Slider(
            ...
            max: 40,
            divisions: 7,
            label: '${_tolerancia.toInt()}%',
            onChanged: (v) => setState(() => _tolerancia = v)),
        ...
        if (filtrado.isEmpty)
          ...
          const SizedBox(height: 12),
          Text('Top 20 postos com maior desvio vs ANP',
              style: TextStyle(
                  fontSize: 11,
          ...
          BarraHorizontal(
            dados: top20
                .map((d) => BarraHorizontalItem(
                      ...
                      texto:
                          '${d.diffPct > 0 ? "+" : ""}${d.diffPct.toStringAsFixed(1)}%',
                    ))
                .toList(),
            eixoX: 'Desvio % vs ANP',
          ),
          ...
          TabelaSimples(
            ...
            linhas: filtrado
                .map((d) => [
                      truncarTexto(d.razaoSocial, 20),
                      d.uf ?? '—',
                      d.combustivel,
                      formatarMoeda(d.precoGf),
                      '${d.diffPct > 0 ? "+" : ""}${d.diffPct.toStringAsFixed(1)}%'
                    ])
                .toList(),
          ),
...
class _ScorePorRegiaoState extends State<_ScorePorRegiao> {
  ...
  Widget build(BuildContext context) {
    ...
    final agrupado = agrupadoMapa.entries
        .map((e) =>
            (chave: e.key, scoreMedio: media(e.value), n: e.value.length))
        .toList()
      ..sort((a, b) => b.scoreMedio.compareTo(a.scoreMedio));
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          ChoiceChip(
              label: const Text('Macrorregião', style: TextStyle(fontSize: 11)),
              selected: !_porUf,
              onSelected: (_) => setState(() => _porUf = false)),
          const SizedBox(width: 6),
          ChoiceChip(
              label: const Text('UF', style: TextStyle(fontSize: 11)),
              selected: _porUf,
              onSelected: (_) => setState(() => _porUf = true)),
        ]),
        const SizedBox(height: 12),
        GridView.count(
          crossAxisCount: 3,
          ...
          children: [
            CartaoIndicador(
                label: '⭐ Score médio',
                valor: scoreGeral.toStringAsFixed(1),
                mini: true),
            CartaoIndicador(
                label: '🏆 Melhor',
                valor: agrupado.isNotEmpty ? agrupado.first.chave : '—',
                mini: true),
            CartaoIndicador(
                label: '⚠️ Pior',
                valor: agrupado.isNotEmpty ? agrupado.last.chave : '—',
                mini: true),
          ],
        ),
        const SizedBox(height: 12),
          ...
          child: BarChart(
            BarChartData(
              ...
              barTouchData: barTouchPadrao(
                  formatarY: (v) => 'Score ${v.toStringAsFixed(1)}'),
              barGroups: agrupado.asMap().entries.map((e) {
                final cor = e.value.scoreMedio >= 70
                    ? const Color(0xFF27AE60)
                    : (e.value.scoreMedio >= 45
                        ? const Color(0xFFF39C12)
                        : const Color(0xFFE74C3C));
                return BarChartGroupData(x: e.key, barRods: [
                  BarChartRodData(
                      toY: e.value.scoreMedio,
                      color: cor,
                      width: 18,
                      borderRadius:
              ...
              titlesData: FlTitlesData(
                ...
                bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (v, _) {
                          final i = v.toInt();
                          if (i < 0 || i >= agrupado.length)
                            return const SizedBox.shrink();
                          return Text(agrupado[i].chave,
                              style: const TextStyle(fontSize: 9));
                        })),
                rightTitles:
        ...
        const SizedBox(height: 12),
        TabelaSimples(
            colunas: const ['Região', 'Score médio', 'Postos'],
            linhas: agrupado
                .map(
                    (a) => [a.chave, a.scoreMedio.toStringAsFixed(1), '${a.n}'])
                .toList()),
      ]),
    );
  }
        ...
        GridView.count(
          ...
          children: ['A', 'B', 'C', 'D'].map((g) {
            ...
            return CartaoIndicador(
                label: '$emoji Categoria $g',
                valor: formatarInt(contagem[g] ?? 0),
                sub:
                    '${((contagem[g] ?? 0) / total * 100).toStringAsFixed(1)}%',
                mini: true);
          }).toList(),
        ),
          ...
          child: PieChart(PieChartData(
            ...
            sections: ['A', 'B', 'C', 'D'].map((g) {
              ...
              return PieChartSectionData(
                  value: v <= 0 ? 0.001 : v.toDouble(),
                  title: '$g ${(v / total * 100).toStringAsFixed(0)}%',
                  color: _coresGrade[g],
                  radius: 75,
                  titleStyle:
        ...
        Builder(builder: (context) {
          ...
          final porUf = porUfMapa.entries.map((e) {
            ...
            return (
              ...
              contagem: e.value,
              totalUf: totalUf,
              pctA: totalUf > 0 ? e.value['A']! / totalUf * 100 : 0.0
            );
          }).toList()
