...
class VeiculosScreen extends StatefulWidget {
  ...
  @override
  State<VeiculosScreen> createState() => _State();
}
...
class _State extends State<VeiculosScreen> {
  ...
  Future<void> _conectarRealtime() async {
    try {
      ...
      _realtimeSub = stream.listen((evento) {
        debugPrint(
            '[REALTIME] Evento recebido: \${evento.tabela} \${evento.evento}');
        if (!mounted) return;
  ...
  List get _veiculosFiltrados {
    if (_filtro == 'cadastrados')
      return _veiculos.where((v) => v['cadastrado'] == true).toList();
    if (_filtro == 'nao_cadastrados')
      return _veiculos.where((v) => v['cadastrado'] != true).toList();
    return _veiculos;
  }
  ...
  Future<void> _novoOuEditar([Map? dados]) async {
    ...
    final anoModCtrl =
        TextEditingController(text: dados?['ano_modelo']?.toString() ?? '');
    final anoFabCtrl =
        TextEditingController(text: dados?['ano_fabricacao']?.toString() ?? '');
    final hodCtrl = TextEditingController(
        text: dados?['hodometro_atual']?.toString() ??
            dados?['hodometro_abast']?.toString() ??
            '');
    final combCtrl = TextEditingController(text: dados?['combustivel'] ?? '');
    final tanqueCtrl =
        TextEditingController(text: dados?['tanque']?.toString() ?? '');
    final autonomiaCtrl =
        TextEditingController(text: dados?['autonomia']?.toString() ?? '');
        ...
        child: Column(children: [
            ...
            child: Row(children: [
              ...
              if (!isCadastrado)
                Container(
                  ...
                  child: const Text('Nao cadastrado',
          ...
          Expanded(
            ...
            child:
              ...
              _campo(placaCtrl, 'Placa *',
                  caps: TextCapitalization.characters, enabled: !isCadastrado),
              Row(children: [
              ...
              Row(children: [
                Expanded(
                    child: _campo(anoModCtrl, 'Ano Modelo',
                    ...
                    onPressed: () async {
                      ...
                      try {
                        final body = {
                          ...
                          'hodometro_atual': double.tryParse(
                              hodCtrl.text.replaceAll(',', '.')),
          ...
          : Column(children: [
              ...
              Padding(
                ...
                child: Row(children: [
                  ...
                  const SizedBox(width: 8),
                  _chipFiltro('Pendentes', 'nao_cadastrados', nNaoCad,
                      cor: Colors.orange),
                ]),
              ),
              // Lista
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              subtitle: Text(
                                cadastrado
                                    ? '${v["marca"] ?? ""} ${v["modelo"] ?? ""} ${v["ano_modelo"] ?? ""}'
                                            ...
                                            .isEmpty
                                        ? v['combustivel'] ?? '-'
                                        : '${v["marca"] ?? ""} ${v["modelo"] ?? ""} ${v["ano_modelo"] != null ? "(${v["ano_modelo"]})" : ""}'
                                            .trim()
                                  ...
                                  children: [
                                    ...
                                    IconButton(
                                      icon: const Icon(Icons.info_outline,
                                          color: Color(0xFF0D2D6B), size: 20),
                              ...
                              onTap: () => VeiculoDetalheModal.show(
                                  context, v['placa'] ?? ''),
                            ),
                          );
                        },
  ...
  Widget _chipFiltro(String label, String valor, int count, {Color? cor}) =>
      GestureDetector(
