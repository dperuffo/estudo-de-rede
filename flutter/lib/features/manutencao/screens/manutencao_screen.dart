...
class ManutencaoScreen extends StatefulWidget {
  ...
  @override
  State<ManutencaoScreen> createState() => _State();
}
...
class _State extends State<ManutencaoScreen>
        ...
        onDeletar: () async {
          ...
          final ok = await showDialog<bool>(
              ...
              builder: (dialogCtx) => AlertDialog(
                    ...
                    content: Text(
                        'Deseja excluir a manutencao do veiculo ${m["placa"]}?'),
                    actions: [
                      TextButton(
  ...
  Future<void> _novoOuEditar([Map? dados]) async {
    ...
    final dataCtrl = TextEditingController(
        text: dados?['data_manutencao'] ??
            DateTime.now().toIso8601String().substring(0, 10));
    final hodCtrl =
        TextEditingController(text: dados?['hodometro']?.toString() ?? '');
    final tecnicoCtrl = TextEditingController(text: dados?['tecnico'] ?? '');
    final oficCtrl = TextEditingController(text: dados?['oficina'] ?? '');
    final custoCtrl =
        TextEditingController(text: dados?['custo_total']?.toString() ?? '');
                  ...
                  child: Column(children: [
                    ...
                    SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () async {
                            ...
                            try {
                              final body = {
                                ...
                                'custo_total': double.tryParse(custoCtrl.text
                                    .replaceAll('R\$', '')
                                    .replaceAll(' ', '')
                                    .replaceAll('.', '')
                                    .replaceAll(',', '.')
                              ...
                              if (id != null) {
                                await ApiService()
                                    .put('/manutencao/$id', data: body);
                              } else {
                                await ApiService()
                                    .post('/manutencao', data: body);
                              }
                              if (ctx.mounted) {
                                Navigator.pop(ctx);
                                _load();
                          ...
                          child: Text(id != null ? 'Salvar' : 'Registrar',
      ...
      : RefreshIndicator(
          ...
          child: ListView(padding: const EdgeInsets.all(16), children: [
            ...
            Row(children: [
              ...
              _cardResumo(
                  'Atencao', _resumoStatus['atencao'] ?? 0, Colors.orange),
              const SizedBox(width: 8),
  ...
  Widget _cardStatusVeiculo(Map v) {
    ...
    final cor = status == 'critico'
        ? Colors.red
        : status == 'atencao'
            ? Colors.orange
            : Colors.green;
    final icon = status == 'critico'
        ? Icons.warning
        : status == 'atencao'
            ? Icons.info
            : Icons.check_circle;
        ...
        child: Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                  color: cor, borderRadius: BorderRadius.circular(10)),
      ...
      child: Column(children: [
        ...
        Expanded(
            child: ListView(
              ...
              if (m['obs_gerais'] != null &&
                  m['obs_gerais'].toString().isNotEmpty) ...[
                _secao('Observacoes'),
                Container(
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
