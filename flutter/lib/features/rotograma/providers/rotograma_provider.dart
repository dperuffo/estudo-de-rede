...
Color corRisco(String categoria) =>
    _corRiscoHex[categoria] ?? _corRiscoHex['perigo']!;
...
String categoriaRiscoLabel(String c) => categoriasRisco
    .firstWhere((x) => x.valor == c, orElse: () => categoriasRisco.first)
    .label;
String categoriaRiscoIcone(String c) => categoriasRisco
    .firstWhere((x) => x.valor == c, orElse: () => categoriasRisco.first)
    .icone;
String categoriaParadaLabel(String c) => categoriasParada
    .firstWhere((x) => x.valor == c, orElse: () => categoriasParada.first)
    .label;
String categoriaParadaIcone(String c) => categoriasParada
    .firstWhere((x) => x.valor == c, orElse: () => categoriasParada.first)
    .icone;
...
double? extrairKmDoLocal(String local) {
  final m =
      RegExp(r'km\s*(\d+(?:[.,]\d+)?)', caseSensitive: false).firstMatch(local);
...
List<PontoLinhaDoTempo> resolverLinhaDoTempo(
    List<RotogramaRisco> riscos, List<RotogramaParada> paradas) {
  final todos = <({
    String tipo,
    String local,
    String categoria,
  ...
  final kmConhecidos = todos
      .map((item) => item.km ?? extrairKmDoLocal(item.local))
      .whereType<double>()
      .toList();
  final kmMaximoConhecido = kmConhecidos.isNotEmpty
      ? kmConhecidos.reduce((a, b) => a > b ? a : b)
      : 100.0;
  ...
  for (var i = 0; i < todos.length; i++) {
    ...
    final km =
        kmExplicito ?? ((i + 1) / (todos.length + 1)) * kmMaximoConhecido;
...
final rotogramasListaProvider =
    FutureProvider.autoDispose<List<RotogramaResumo>>((ref) async {
  ...
  final rows = await SupabaseService.client
      .from('rotogramas')
      .select(
          'id, numero, origem, destino, motorista, placa, data_viagem, criado_em')
      .eq('empresa_id', empresaId)
      .order('criado_em', ascending: false) as List;
  return rows
      .map((r) => RotogramaResumo.fromMap(r as Map<String, dynamic>))
      .toList();
});
...
class RotogramaDetalhe {
  ...
  factory RotogramaDetalhe.fromMap(Map<String, dynamic> m) => RotogramaDetalhe(
        ...
        criadoEm: m['criado_em'] as String,
        riscos: ((m['riscos'] as List?) ?? [])
            .map((r) => RotogramaRisco.fromMap(r as Map<String, dynamic>))
            .toList(),
        paradas: ((m['paradas'] as List?) ?? [])
            .map((p) => RotogramaParada.fromMap(p as Map<String, dynamic>))
            .toList(),
      );
}

final rotogramaDetalheProvider = FutureProvider.autoDispose
    .family<RotogramaDetalhe?, String>((ref, id) async {
