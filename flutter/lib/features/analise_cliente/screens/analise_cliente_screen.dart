...
class AnaliseClienteScreen extends StatefulWidget {
  ...
  @override
  State<AnaliseClienteScreen> createState() => _State();
}

class _State extends State<AnaliseClienteScreen>
  ...
  Future<void> _load() async {
    ...
    try {
      final r =
          await ApiService().get('/analise-cliente', params: {'dias': _dias});
      setState(() => _dados = r);
    } catch (e) {
      if (mounted)
      ...
      child: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        Container(
          ...
          child: Column(children: [
            ...
            const Text('Total em combustivel',
                style: TextStyle(color: Colors.white70, fontSize: 13)),
            const SizedBox(height: 16),
            Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
              _metrica('${kpis["n_abastecimentos"] ?? 0}', 'Abastec.'),
              _metrica('${(kpis["total_litros"] ?? 0).toStringAsFixed(0)} L',
                  'Litros'),
        ...
        GridView.count(
          ...
          children: [
            _kpiCard('Veiculos', '${kpis["n_veiculos"] ?? 0}',
                Icons.directions_car, Colors.blue),
            _kpiCard('Motoristas', '${kpis["n_motoristas"] ?? 0}', Icons.person,
                Colors.teal),
            _kpiCard('Postos', '${kpis["n_postos"] ?? 0}',
                Icons.local_gas_station, Colors.green),
            _kpiCard('Preco Medio', fmt.format(kpis['preco_medio'] ?? 0),
                Icons.price_change, Colors.orange),
            _kpiCard(
                'Media/Veic',
                fmt.format(
                    (kpis['total_gasto'] ?? 0) / (kpis['n_veiculos'] ?? 1)),
                Icons.analytics,
                Colors.purple),
            _kpiCard(
                'Litros/Abast',
                '${((kpis["total_litros"] ?? 0) / (kpis["n_abastecimentos"] ?? 1)).toStringAsFixed(0)} L',
                Icons.water_drop,
                Colors.cyan),
          ],
        ),
          ...
          child:
            ...
            Row(children: [
              ...
              if ((v["hodometro_max"] as num? ?? 0) > 0)
                _tag('${NumberFormat('#,##0').format(v["hodometro_max"])} km',
                    Colors.orange),
              const Spacer(),
  ...
  Widget _buildMotoristas(NumberFormat fmt, List dados, Map kpis) {
    if (dados.isEmpty)
      return const Center(child: Text('Nenhum motorista identificado'));
    final total = (kpis['total_gasto'] as num? ?? 1).toDouble();
    return ListView(padding: const EdgeInsets.all(16), children: [
                  ...
                  children: dados.take(6).toList().asMap().entries.map((e) {
                    ...
                    return Padding(
                      ...
                      child: Row(children: [
                        ...
                        const SizedBox(width: 6),
                        Text(
                            '${e.value["pv_uf"]} — ${fmt.format(e.value["gasto"] ?? 0)}',
        ...
        return Padding(
          ...
          child:
            ...
            Padding(
              ...
              child: Text(
                  '${(u["litros"] ?? 0).toStringAsFixed(0)} L · ${u["n"]} abast.',
                  style: TextStyle(fontSize: 11, color: Colors.grey[600])),
            ),
          ]),
        );
  ...
  Widget _graficoBarras(List dados, NumberFormat fmt) {
    final maxVal = dados
        .map((d) => (d['gasto'] as num? ?? 0).toDouble())
        .reduce((a, b) => a > b ? a : b);
  ...
  Widget _barraHorizontal(String label, double valor, double total,
      NumberFormat fmt, String sub, Color color) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 12),
