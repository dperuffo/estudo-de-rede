...
class ComeceSeuDiaScreen extends StatefulWidget {
  ...
  @override
  State<ComeceSeuDiaScreen> createState() => _State();
}
...
class _State extends State<ComeceSeuDiaScreen> {
              ...
              child: ListView(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                        ...
                        children: [
                          ...
                          Text(
                              'Resumo operacional — ${_periodo == "ontem" ? "ontem" : _dias == 1 ? "hoje" : "ultimos $_dias dias"}',
                              style: const TextStyle(
                  ...
                  Row(children: [
                    _kpiCard(
                        'Abastecimentos',
                        '${kpis["n_abastecimentos"] ?? 0}',
                        Icons.local_gas_station,
                        Colors.blue),
                    const SizedBox(width: 8),
                    _kpiCard(
                        'Litros',
                        '${(kpis["total_litros"] ?? 0).toStringAsFixed(0)} L',
                        Icons.water_drop,
                        Colors.cyan),
                  ]),
                  const SizedBox(height: 8),
                  Row(children: [
                    _kpiCard(
                        'Gasto Combustivel',
                        fmt.format(kpis['total_gasto'] ?? 0),
                        Icons.payments,
                        Colors.green),
                    const SizedBox(width: 8),
                    _kpiCard(
                        'Preco Medio/L',
                        fmt.format(kpis['preco_medio'] ?? 0),
                        Icons.price_change,
                        Colors.orange),
                  ]),
                  const SizedBox(height: 8),
                  Row(children: [
                    _kpiCard(
                        'Ticket Medio',
                        fmt.format(kpis['ticket_medio'] ?? 0),
                        Icons.receipt,
                        Colors.purple),
                    const SizedBox(width: 8),
                    _kpiCard('Veiculos Ativos', '${kpis["n_veiculos"] ?? 0}',
                  ...
                  Row(children: [
                    _kpiCard(
                        'Manutencao',
                        fmt.format(kpis['total_manutencao'] ?? 0),
                        Icons.build,
                        Colors.red),
                    const SizedBox(width: 8),
                    _kpiCard(
                  ...
                  if (ultimos.isNotEmpty) ...[
                    ...
                    ...ultimos.map((a) => Card(
                          ...
                          child: ListTile(
                            ...
                            title: Text(
                                '${a["veiculo_placa"] ?? "-"} — ${a["item_nome"] ?? "-"}',
                                style: const TextStyle(
                                    fontWeight: FontWeight.bold, fontSize: 13)),
                            subtitle: Text(
                                '${a["data_abastecimento"]?.toString().substring(0, 10) ?? "-"} · ${a["pv_municipio"] ?? "-"}/${a["pv_uf"] ?? "-"}'),
                            trailing: Column(
                                ...
                                children: [
                                  Text(fmt.format(a['item_valor_total'] ?? 0),
                                      style: const TextStyle(
                                          ...
                                          fontSize: 12)),
                                  Text(
                                      '${(a["item_quantidade"] ?? 0).toStringAsFixed(0)} L',
                                      style: TextStyle(
                                          fontSize: 11,
                                          color: Colors.grey[600])),
                                ]),
                            onTap: () =>
                                AbastecimentoDetalheModal.show(context, a),
                          ),
                        )),
                  ],
                ],
  ...
  Widget _barraHorizontal(
      String label, num valor, num total, NumberFormat fmt, String sub) {
  ...
  Widget _itemRanking(
          int pos, String titulo, String valor, String sub, Color color) =>
      Container(
        margin: const EdgeInsets.only(bottom: 8),
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(
