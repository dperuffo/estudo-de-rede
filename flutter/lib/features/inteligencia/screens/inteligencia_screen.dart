...
class InteligenciaScreen extends StatefulWidget {
  ...
  @override
  State<InteligenciaScreen> createState() => _State();
}
  ...
  Future<void> _load() async {
    ...
    try {
      final r = await ApiService()
          .get('/inteligencia/resumo', params: {'dias': _dias});
      setState(() => _dados = r);
    } catch (e) {
      if (mounted)
      ...
      body: _loading
          ...
          : RefreshIndicator(
              ...
              child: Builder(builder: (_) {
                final lista = _tabIndex == 0
                    ? porUf
                    : _tabIndex == 1
                        ? porMun
                        : porVei;
                final keyField = _tabIndex == 0
                    ? 'pv_uf'
                    : _tabIndex == 1
                        ? 'pv_municipio'
                        : 'veiculo_placa';
                if (lista.isEmpty)
                  return const Center(child: Text('Sem dados para o periodo'));
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(
                            '${(item["litros"] ?? 0).toStringAsFixed(0)} L · ${item["n"]} abast.'),
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              border: Border(
                  bottom: BorderSide(
                      color:
                          _tabIndex == idx ? Colors.white : Colors.transparent,
                      width: 2)),
            ),
            child: Text(label,
