...
class IndicadoresFrotaScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<IndicadoresFrotaScreen> createState() =>
      _IndicadoresFrotaScreenState();
}

class _IndicadoresFrotaScreenState
    extends ConsumerState<IndicadoresFrotaScreen> {
  ...
  Widget _corpo(AsyncValue<KpisFrota?> kpisAsync, List<VeiculoKpi> veiculos,
      FiltroKpisFrota filtro) {
    final tipos = veiculos
        .map((v) => v.tipoVeiculo)
        .whereType<String>()
        .toSet()
        .toList()
      ..sort();
    final modelos = veiculos
        .map((v) => v.modelo)
        .whereType<String>()
        .toSet()
        .toList()
    ...
    final filtrados = veiculos
        .where((v) =>
            (_tipoVeiculo == null || v.tipoVeiculo == _tipoVeiculo) &&
            (_modelo == null || v.modelo == _modelo))
        .toList();
    ...
    if (veiculoSelecionado != null) {
      ...
      contexto =
          'Frota filtrada (${filtrados.length} veículo${filtrados.length == 1 ? '' : 's'})';
    } else {
        ...
        DropdownButtonFormField<String?>(
          ...
          items: [
            const DropdownMenuItem(
                value: null, child: Text('Todos (agregado)')),
    ...
    return Column(
      children: [
        GridView.count(
          ...
          crossAxisSpacing: 8,
          children: [
            _card('Disponibilidade',
                k.disponibilidadePct != null ? '${k.disponibilidadePct}%' : '—',
                destaque:
                    k.disponibilidadePct != null && k.disponibilidadePct! < 90),
            _card(
                'CPK operacional',
                k.cpkOperacional != null
                    ? '${_moeda.format(k.cpkOperacional)}/km'
                    : '—'),
            _card('Consumo médio',
                k.mediaKmL != null ? '${k.mediaKmL} km/l' : '—'),
            _card('Utilização',
                k.utilizacaoPct != null ? '${k.utilizacaoPct}%' : '—',
                destaque: k.utilizacaoPct != null && k.utilizacaoPct! < 70),
            _card(
            ...
            _card(veiculoSelecionado != null ? 'Veículo' : 'Veículos no filtro',
                veiculoSelecionado?.placa ?? '${k.totalVeiculos}'),
            _card(
              'Conformidade (checklist)',
              k.conformidadePct != null
                  ? '${k.conformidadePct}%'
                  : 'Sem inspeções',
            ...
            if (k.indiceSinistralidade != null)
              _card('Índice de sinistralidade', '${k.indiceSinistralidade}%',
                  destaque: k.indiceSinistralidade! > 10)
            else
              _card('Sinistros no período', '${k.totalSinistros}',
                  destaque: k.totalSinistros > 0),
          ],
        ),
        if (k.manutencaoNaoClassificadaCusto > 0) ...[
          ...
          TextButton(
            ...
            child: const Text('Ir para Manutenção Preditiva →',
  ...
  Widget _tabelaComparacao(List<VeiculoKpi> veiculos) {
    ...
    switch (_sortColumnIndex) {
      ...
      case 1:
        ordenados
            .sort((a, b) => cmp(a.disponibilidadePct, b.disponibilidadePct));
        break;
      case 2:
      ...
      child: DataTable(
        ...
        columns: [
          DataColumn(
              ...
              onSort: (i, _) => ordenarPor(i)),
          DataColumn(
              label: const Text('Disp.', style: TextStyle(fontSize: 12)),
              numeric: true,
              onSort: (i, _) => ordenarPor(i)),
          DataColumn(
              label: const Text('CPK', style: TextStyle(fontSize: 12)),
              numeric: true,
              onSort: (i, _) => ordenarPor(i)),
          DataColumn(
              label: const Text('Consumo', style: TextStyle(fontSize: 12)),
              numeric: true,
              onSort: (i, _) => ordenarPor(i)),
          DataColumn(
              label: const Text('Utiliz.', style: TextStyle(fontSize: 12)),
              numeric: true,
              onSort: (i, _) => ordenarPor(i)),
          DataColumn(
              label: const Text('Conf.', style: TextStyle(fontSize: 12)),
              numeric: true,
              onSort: (i, _) => ordenarPor(i)),
          DataColumn(
              label: const Text('Sinistr.', style: TextStyle(fontSize: 12)),
              numeric: true,
              onSort: (i, _) => ordenarPor(i)),
        ],
        rows: lista
            .map((v) => DataRow(
                  ...
                  onSelectChanged: (_) => setState(() => _placaSelecionada =
                      _placaSelecionada == v.placa ? null : v.placa),
                  cells: [
                    DataCell(Text(v.placa,
                        style: const TextStyle(
                            fontSize: 12, fontWeight: FontWeight.w700))),
                    DataCell(Text(
                        v.disponibilidadePct != null
                            ? '${v.disponibilidadePct}%'
                            : '—',
                        style: const TextStyle(fontSize: 12))),
                    DataCell(Text(
                        v.cpkOperacional != null
                            ? _moeda.format(v.cpkOperacional)
                            : '—',
                        style: const TextStyle(fontSize: 12))),
                    DataCell(Text(v.mediaKmL != null ? '${v.mediaKmL}' : '—',
                        style: const TextStyle(fontSize: 12))),
                    DataCell(Text(
                        v.utilizacaoPct != null ? '${v.utilizacaoPct}%' : '—',
                        style: const TextStyle(fontSize: 12))),
                    DataCell(Text(
                        v.conformidadePct != null
                            ? '${v.conformidadePct}%'
                            : '—',
                        style: const TextStyle(fontSize: 12))),
                    DataCell(Text('${v.totalSinistros}',
                        style: const TextStyle(fontSize: 12))),
                  ],
                ))
            .toList(),
