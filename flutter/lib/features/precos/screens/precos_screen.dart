...
class PrecosScreen extends StatefulWidget {
  ...
  @override
  State<PrecosScreen> createState() => _State();
}
...
class _State extends State<PrecosScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final r =
          await ApiService().get('/precos/variacao', params: {'dias': _dias});
      setState(() => _dados = r);
    } catch (e) {
      if (mounted)
  ...
  Widget _cardCombustivel(Map c, NumberFormat fmt) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Expanded(
              child: _precoBadgeGrande(
                  'Medio', preco, fmt, isAlto ? Colors.red : Colors.green)),
        ...
        Row(children: [
          Text(fmt.format(pMin),
              style: const TextStyle(fontSize: 10, color: Colors.green)),
          const Spacer(),
          Text(fmt.format(pMax),
  ...
  Widget _precoBadgeGrande(
          String label, double valor, NumberFormat fmt, Color color) =>
      Column(children: [
  ...
  Widget _graficoLinha(List serie, List porComb, NumberFormat fmt) {
    ...
    final meses = serie.map((s) => s['mes']?.toString() ?? '').toSet().toList()
      ..sort();
    ...
    for (int ci = 0; ci < combustiveis.length && ci < 4; ci++) {
      ...
      for (int mi = 0; mi < meses.length; mi++) {
        ...
        if (entry.isNotEmpty) {
          spots.add(FlSpot(
              mi.toDouble(), (entry['preco_medio'] as num? ?? 0).toDouble()));
        }
      }
      if (spots.isNotEmpty) {
        linhas.add(LineChartBarData(
          ...
          belowBarData: BarAreaData(
              show: true, color: cores[ci % cores.length].withOpacity(0.05)),
        ));
      }
    }
    ...
    return Column(children: [
              ...
              touchTooltipData: LineTouchTooltipData(
                getTooltipItems: (spots) => spots
                    .map((s) => LineTooltipItem(
                        fmt.format(s.y),
                        TextStyle(
                            color: cores[s.barIndex % cores.length],
                            fontSize: 11)))
                    .toList(),
              ),
            ),
      ...
      Wrap(
          ...
          children: List.generate(
              combustiveis.length > 4 ? 4 : combustiveis.length,
              (i) => Row(mainAxisSize: MainAxisSize.min, children: [
                    Container(
                        width: 12, height: 3, color: cores[i % cores.length]),
