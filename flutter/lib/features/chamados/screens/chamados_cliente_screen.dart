...
// Fase FLT-3 — Chamados (cliente): cópia quase 1:1 de
// chamados_posto_screen.dart (FLT-2), reaproveitando DIRETO
// `chamadosPostoProvider`/`Ticket`/`statusTicket`/`tiposTicket` do Posto —
// o provider já é genérico (só lê `sessao.empresaId`, sem bifurcação por
// segmento; ver comentário em chamados_provider.dart). A única diferença
// real são as rotas de navegação (aqui sem o prefixo `/posto`), por isso
// não deu pra reaproveitar a TELA em si (que tem `/posto/chamados/...`
// hardcoded) — mesmo espírito do `PostoCobrancaDetalheScreen`/drill-downs
// financeiros: provider 100% compartilhado, UI copiada e adaptada.
class ChamadosClienteScreen extends ConsumerStatefulWidget {
  const ChamadosClienteScreen({super.key});

  @override
  ConsumerState<ChamadosClienteScreen> createState() => _ChamadosClienteScreenState();
}

class _ChamadosClienteScreenState extends ConsumerState<ChamadosClienteScreen> {
