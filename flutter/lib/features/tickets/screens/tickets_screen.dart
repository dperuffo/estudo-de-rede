...
class TicketsScreen extends StatefulWidget {
  ...
  @override
  State<TicketsScreen> createState() => _State();
}
...
class _State extends State<TicketsScreen> {
  ...
  Widget _kpiTicket(String label, int count, Color color, IconData icon) =>
      Expanded(
        child: Container(
                          ...
                          child: Column(children: [
                            ...
                            DropdownButtonFormField<String>(
                              ...
                              items: const [
                                DropdownMenuItem(
                                    value: 'incidente',
                                    child: Text('Incidente / Erro')),
                                DropdownMenuItem(
                                    value: 'melhoria', child: Text('Melhoria')),
                              ],
              ...
              child: Column(children: [
                ...
                if (_tickets.isNotEmpty)
                  Padding(
                    ...
                    child: Row(children: [
                      _kpiTicket('Abertos', _resumo['abertos']!, Colors.blue,
                          Icons.fiber_new),
                      const SizedBox(width: 8),
                      _kpiTicket('Em Analise', _resumo['analise']!,
                          Colors.orange, Icons.hourglass_empty),
                      const SizedBox(width: 8),
                      _kpiTicket('Resolvidos', _resumo['resolvidos']!,
...
class _DetalheTicket extends StatefulWidget {
  ...
  @override
  State<_DetalheTicket> createState() => _DetalheState();
}
...
class _DetalheState extends State<_DetalheTicket> {
  ...
  Widget build(BuildContext context) {
    ...
    final status = t['status'] as String?;
    final corStatus = status == 'resolvido'
        ? Colors.green
        : status == 'em_analise'
            ? Colors.orange
            : status == 'fechado'
      ...
      child: Column(children: [
        ...
        Expanded(
            child: ListView(
              ...
              // Resposta admin
              if (t['resposta_admin'] != null &&
                  t['resposta_admin'].toString().isNotEmpty) ...[
                ...
                Container(
                  ...
                  child: Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Expanded(
                            child: Text(t['resposta_admin'].toString(),
                                style: const TextStyle(fontSize: 14))),
                      ]),
                ),
                const SizedBox(height: 16),
              ...
              // Anexos
              if ((t['anexos'] != null) &&
                  _parseAnexos(t['anexos']).isNotEmpty) ...[
                const Text('Anexos',
                    style: TextStyle(
                        fontWeight: FontWeight.bold,
                        color: Color(0xFF0D2D6B),
                      ...
                      child: Row(children: [
                        ...
                        if (a['conteudo'] != null)
                          TextButton(
                            onPressed: () async {
                              final url =
                                  'data:${a["tipo_mime"] ?? "application/octet-stream"};base64,${a["conteudo"]}';
